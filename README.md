# 🌐 Jason Putra Wijaya Portfolio Website

Jason Putra Wijaya is a web and game developer specializing in full-stack web development, Android applications, and Unity game development.  
This portfolio showcases projects, skills, and technology stacks used in modern web and game systems.

---

## 🚀 About This Project

This is my personal portfolio website built to showcase my work in:

- 🌍 Full-stack web development  
- ⚙️ Backend APIs and system engineering  
- 📱 Android application development  
- 🎮 Unity 2D & 3D game development (C#)  

The website is designed to be fast, modern, responsive, and developer-focused, with bilingual support (English & Indonesian).

---

## 🧩 Features

- ✅ Responsive modern UI (Desktop, Tablet, Mobile)
- 🌐 Automatic language system (EN / ID)
- 🎮 Dynamic role animation text
- 🧩 Projects showcase section
- 📊 Skills running marquee animation
- 📩 Contact section with FontAwesome icons
- 🍔 Mobile burger navigation with blur overlay
- ⚡ Lightweight (No framework, pure HTML/CSS/JS)
- 🌙 Dark mode futuristic design

---

## 🛠️ Tech Stack

**Frontend**
- HTML5  
- CSS3 (Responsive, Glass UI, Animations)  
- JavaScript (Vanilla JS)  

**Assets & Libraries**
- Google Fonts (Inter)  
- Font Awesome Icons  

---

## 🎮 Featured Projects

- **Stickman Bross** – 2D Unity action game  
- **Rahasia Gang Sekolah** – Indonesian mystery story game  
- **Defense Idle** – Idle tower defense prototype

### 🌐 Website Projects
- **X1Creeper** – Official website of class X-1 SMA Bonaventura Madiun  
  👉 https://jproject-1.github.io/x1creeper/

- **GUP Harvest** – Church website with news, gallery, and events  
  👉 https://gupharvest.web.id/

- **UMC Project** – Multimedia Student Activity Unit website at Universitas Stikubank Semarang
  👉 https://umcproject.ct.ws/

---

## 🌍 Live Demo

🔗 [https://jproject-1.github.io/portfolio/](https://jproject-1.github.io/portfolio/) 

---

## 📬 Contact

- 📧 Email: jpwprojects1@gmail.com  
- 📸 Instagram: [https://www.instagram.com/json_acc/](https://www.instagram.com/json_acc/)

---

## 📜 License

This project is open-source and free to use for learning purposes.  
Please give credit if you use the design or code.

---

## ✨ Author

**Jason Putra Wijaya**  
Web & Game Developer | Co-Founder of YCYL Studio  
Indonesia 🇮🇩
